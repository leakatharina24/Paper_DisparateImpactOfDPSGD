...
class BaseTrainer:
    ...
    def _train_epoch(self, cosine_sim_per_epoch, expected_loss, param_for_step=None):
        ...
        for _batch_idx, (data, target, group) in enumerate(self.train_loader): #tqdm(
    ...
    def train(self, write_checkpoint=True):
        ...
        while self.epoch < self.max_epochs:
            ...
            #group_performance_epochs.append([self.epoch, valid_results["accuracy"]] + valid_results['accuracy_per_group'] + \
            #                                [valid_results["roc_auc"]] + valid_results["roc_auc_per_group"])
            
            if self.early_stopping:
                if valid_results["accuracy"] <= best_accuracy-min_delta:
                    ...
                    best_accuracy = valid_results['accuracy'] if valid_results['accuracy'] > best_accuracy else best_accuracy
                    valid_results_previous = valid_results
        ...
        #K = self.num_groups
        ## write group performance metrics to csv
        #columns = ["epoch", "accuracy"] + [f"accuracy_{k}" for k in range(K)] + ["roc_auc"] + [f"roc_auc_{k}" for k in range(K)]
        #self.create_csv(group_performance_epochs, columns, "valid_performance_per_epochs")
        ...
        return valid_results_previous, test_results #added by LD
    ...
    def _validate(self):
        ...
        return valid_results #added by LD
    ...
    def _test(self):
        ...
        #                  scale_to_01=1)
        return test_results #added by LD
    ...
    def expected_loss(self, g_B, bar_g_B, sum_grad_vec, grad_hess_grad, clip_grad_hess_clip_grad,
        ...
        for group_idx in range(groups_len):
            g_D_a = sum_grad_vec[group_idx] / int(group_counts[group_idx])
...
class DpsgdTrainer(BaseTrainer):
    ...
    def clipping_scale_fn(self, grad_norm, idx, clipping_bound):
        try:
            clipped_grad_norm = min(1, clipping_bound / grad_norm)
...
class FairnessLensTrainer(BaseTrainer):
    """Class for DPSGD training with regularized loss function from 'Differentially Private Empirical Risk Minimization under the Fairness Lens'"""
    ...
    def _clear_backprops(self, model: torch.nn.Module) -> None:
    ...
    def _train_epoch(self, cosine_sim_per_epoch, expected_loss, param_for_step=None):
        ...
        unreduced_criterion = torch.nn.CrossEntropyLoss(reduction='none')
        criterion = torch.nn.CrossEntropyLoss()
        losses = []
        losses_per_group = np.zeros(self.num_groups)
        ...
        for _batch_idx, (data, target, group) in enumerate(self.train_loader): #tqdm(
            ...
            # Do computations for tracking history
            self.optimizer.zero_grad()
            output = self.model(data)
            loss = criterion(output, target)
            loss.backward()
            ...
            g_B, g_B_k, bar_g_B, bar_g_B_k = self.mean_grads_over(group_counts_batch, sum_grad_vec_batch,
                                                                  sum_clip_grad_vec_batch)
            if (self.evaluate_angles or self.evaluate_hessian) and (
                    self.epoch * len(self.train_loader) + _batch_idx) % self.angle_comp_step == 0:
                # compute cosine simularity bw g_D_k and g_B_k and bar_g_B_k
                sum_grad_vec_all, sum_clip_grad_vec_all, group_counts = self.get_sum_grad(self.train_loader.dataset,
                                                                                          criterion,
                                                                                          g_B,
                                                                                          bar_g_B,
                ...
                # average sum of gradients per group
                _, g_D_k, _, _ = self.mean_grads_over(group_counts, sum_grad_vec_all, sum_clip_grad_vec_all)
                # compute sum of gradients over groups over entire training dataset
                cosine_sim_per_epoch.append(self.evaluate_cosine_sim(_batch_idx, g_D_k, g_B, bar_g_B, g_B_k, bar_g_B_k))

            # Reset and do computation for SGD step
            self.optimizer.zero_grad()
            output = self.model(data)
            ...
            CE_losses_per_group = np.zeros(self.num_groups)
            # Step 1: Compute gradient of loss function wrt model parameters for each group of samples
            loss_list = unreduced_criterion(output, target)
            self.model.disable_hooks()
            for i in range(self.num_groups):
                self.optimizer.zero_grad()
                self._clear_backprops(self.model)
                group_loss = torch.mean(loss_list[group == i])
                CE_losses_per_group[i] += group_loss.item()
                group_loss_grad = torch.autograd.grad(group_loss, self.model.parameters(), create_graph=True)
                grad_group_dict[i] = torch.cat([torch.flatten(p) for p in group_loss_grad]).view(-1)
                num_samples_per_group_list.append(
                    float(len(loss_list[group == i])))  # number of samples of group-i in this mini-batch
                if flatten_grad is None:
                    flatten_grad = grad_group_dict[i] * num_samples_per_group_list[i]
                else:
                    flatten_grad = flatten_grad + grad_group_dict[i] * num_samples_per_group_list[i]
            flatten_grad = flatten_grad / np.sum(num_samples_per_group_list)

            # Step 2: Compute flattened average of clipped gradient
            flatten_clipped_grad = torch.zeros(flatten_grad.shape).to(self.device)
            for ind_loss in loss_list:
                self.optimizer.zero_grad()
                self._clear_backprops(self.model)
                ind_loss_grad = torch.autograd.grad(ind_loss, self.model.parameters(), create_graph=True)
                ind_grad_norm = torch.norm(torch.stack([torch.norm(p) for p in ind_loss_grad]))
                flatten_ind_clipped_grad = torch.cat([torch.flatten(p) for p in ind_loss_grad]).view(-1) \
                                           * torch.min(torch.tensor([1.0]).to(self.device),
                                                       self.optimizer.max_grad_norm / ind_grad_norm)
                flatten_clipped_grad += flatten_ind_clipped_grad / np.sum(num_samples_per_group_list)

            grad_losses_per_group = torch.zeros(self.num_groups).to(self.device)
            for i in range(self.num_groups):
                grad_losses_per_group[i] += torch.dot(grad_group_dict[i], flatten_clipped_grad - flatten_grad)
            if self.num_groups == 2:
                reg_loss_1 = torch.abs(grad_losses_per_group[0] - grad_losses_per_group[1])
            else:
                raise NotImplementedError(
                    "The unfairness mitigation method from 'Fairness Lens' paper was only implemented by the authors for two protected groups.")

            boundary_losses_per_group = torch.zeros(self.num_groups).to(self.device)
            for i in range(self.num_groups):
                boundary_losses_per_group[i] += torch.mean(output[group == i] * (1 - output[group == i]))
            if self.num_groups == 2:
                reg_loss_2 = torch.abs(boundary_losses_per_group[0] - boundary_losses_per_group[1])
            ...
            total_loss = clf_loss + (self.gradient_regularizer * reg_loss_1) + (self.boundary_regularizer * reg_loss_2)
            ...
            for i in range(self.num_groups):
                if len(grad_norms[i]) != 0:
                    ...
                    group_max_grads[i] = max(group_max_grads[i], max(grad_norms[i]))
                    g_B_k_norms[i].append(torch.linalg.norm(g_B_k[i]).item())
            # For SGD update step use regularized loss
            self.optimizer.step()
            # For plotting only use the CE loss
            losses_per_group += CE_losses_per_group
